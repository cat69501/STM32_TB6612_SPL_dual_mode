...
#define MOTOR_SPEED_NORMAL    2100    // 正常速度 (约55%功率)-->30% by YH
#define MOTOR_SPEED_TURN      2100    // 转向速度 (约42%功率)-->30% by YH

#define RC_DEADBAND           5       // RC遥控器死区(百分比)
#define RC_TIMEOUT_MS         1000    // RC信号超时时间(ms)

#define AUTO_ACTION_DURATION  500    // 自动模式每个动作持续时间(ms)
#define AUTO_PAUSE_DURATION   500    // 自动模式动作间暂停时间(ms)
...
void RC_DifferentialControl(int16_t throttle_percent, int16_t steering_percent)
{
   ...
   // DifferentialDrive_SetSpeed(left_speed, right_speed);
	    DifferentialDrive_SetSpeed(right_speed, left_speed); //特殊处理，特定小车
}
